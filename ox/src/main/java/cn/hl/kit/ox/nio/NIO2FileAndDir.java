...
public class NIO2FileAndDir {
	...
	public static void main(String[] args) {
		Path path = FileSystems.getDefault().getPath(System.getProperty("user.home"), "www", "pyweb.settings");
		...
		// 检测文件是否指定同一个文件
		Path path_1 = FileSystems.getDefault().getPath(System.getProperty("user.home"), "www", "pyweb.settings");
		Path path_2 = FileSystems.getDefault().getPath(System.getProperty("user.home"), "www", "django.wsgi");
		Path path_3 = FileSystems.getDefault().getPath(System.getProperty("user.home"), "software/../www", "pyweb.settings");
		try {
		...
		Path newdir2 = FileSystems.getDefault().getPath("/tmp/aaa", "/bbb/ccc/ddd");
		try {
		...
		try (DirectoryStream<Path> ds = Files.newDirectoryStream(newdir3, "*.{png,jpg,bmp，ini}")) {
			for (Path file : ds) {
				System.out.println(file.getFileName());
			}
		} catch (IOException e) {
		...
		// 创建新文件
		Path newfile = FileSystems.getDefault().getPath("/tmp/SonyEricssonOpen.txt");
		Set<PosixFilePermission> perms1 = PosixFilePermissions.fromString("rw-------");
		try {
			FileAttribute<Set<PosixFilePermission>> attr2 = PosixFilePermissions.asFileAttribute(perms1);
		...
		Path copy_to = Paths.get("/tmp/bbb", copy_from.getFileName().toString());
		try {
