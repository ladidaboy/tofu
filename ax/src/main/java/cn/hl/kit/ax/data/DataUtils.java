...
import cn.hl.kit.ax.CommonConst.CompareFeature;
import cn.hl.kit.ax.CommonConst.FormatPrettify;
...
import cn.hl.kit.spring.SpringUtils;
...
public class DataUtils {
    ...
    public static boolean isValidDateTime(String datetime) {
        return isValidDateFormat(datetime, CommonConst.F_DATETIME) // 年-月-日 时:分:秒
                || isValidDateFormat(datetime, CommonConst.F_DATE) // 年-月-日
                || isValidDateFormat(datetime, CommonConst.F_DATETIME_MILLIS); // // 年-月-日 时:分:秒.毫秒
    }
    ...
    public static String convert2String(Object value) {
        ...
        if (value instanceof Date) {
            return CommonConst.SDF_DTT.format(value);
        } else {
    ...
    public static String formatValue(Object value, FormatPrettify... prettifies) {
        ...
        if (value == null) {
            ...
            if (isValidDateTime(value.toString())) {
                ...
                ss = CommonConst.SDF_DAT.format((Date) value);
            } else {
                ss = CommonConst.SDF_DTT.format((Date) value);
            }
     ...
     * @return 格式后的字符串
     */
    public static String formatInteger(String value, String formatter) {
        if (value == null) {
            return null;
        }
    ...
    public static String humpToLine(String attr) {
        ...
        matcher.appendTail(sb);
        if (StringUtils.startsWith(sb, "_")) {
    ...
    public static int randomInt(int min, int max, String cycle) {
        ...
        if (cycle != null && !cycle.trim().equals("")) {
            ArrayList<Integer> numbers = RANDOM_NUMBERS.get(cycle);
            if (numbers == null) {
                numbers = new ArrayList<Integer>();
                RANDOM_NUMBERS.put(cycle, numbers);
            }
            if (numbers.isEmpty()) {
    ...
    public static void main(String[] args) {
        ...
        bean.setId("No.99");
        System.out.println(bean + " - Valid: " + isValidBean(bean));
        bean.setAge(9999);
        System.out.println(bean + " - Valid: " + isValidBean(bean));
        //
        ...
        try {
            ...
            for (int i = 0; i < maxSize; i++) {
                dbs.add(getScale(Math.random() * 10000, 2));
            }
            Double indexValue = getPercentIndex(dbs, 78.9);
            System.err.println("Index(78.9%) = " + indexValue);
        ...
        try {
            ...
            Long indexValue = getPercentIndex(dbs, 88.8);
            System.err.println("Index(88.8%) = " + indexValue);
        ...
        try {
            LogUtils.printSplitLine(lenSize);
            Date date = DateUtils.parseDate("2019-09-26 12:34:56", "yyyy-MM-dd HH:mm:ss'Z'", "yyyy-MM-dd HH:mm:ss");
            System.out.println(date);
        } catch (ParseException e) {
            LogUtils.printError("parseDate", e);
        }

        LogUtils.printHeadlineEx("Hump <-> Line", lenSize);
        ...
        try {
            ...
            System.out.println("isInternalIp(" + ip + ") >> " + isInternalIp(ip));
            ip += "/23";
            System.out.println("isInternalIp(" + ip + ") >> " + isInternalIp(ip));
            ip = "192.168.128.0/23";
            System.out.println("isInternalIp(" + ip + ") >> " + isInternalIp(ip));
            ...
            System.out.println("ipInSegment(" + ip + ", " + segment + ") >> " + ipInSegment(ip, segment));
            ip = "192.168.65.234";
            segment = "192.168.128.0/19";
            System.out.println("ipInSegment(" + ip + ", " + segment + ") >> " + ipInSegment(ip, segment));

            LogUtils.printSplitWave(lenSize);
            getIpSegmentRange("10.10.0.0/19");
            getIpSegmentRange("10.10.0.0/29");
            //getIpSegmentRange("10.10.0.3/30");
            getIpSegmentRange("10.10.0.0/31");
            getIpSegmentRange("10.10.0.0/32");
            LogUtils.printCornerTitleEx("getIpSegmentRange", lenSize);
        } catch (Exception e) {
            LogUtils.printError("Test IP API", e);
        }

        System.out.println("matchOne: " + matchOne(FormatPrettify.ENUM, FormatPrettify.NONE, FormatPrettify.ENUM));
        System.out.println("enum.findIn: " + FormatPrettify.ENUM.findIn(FormatPrettify.NONE, FormatPrettify.DATE));

        Date now = new Date();
        System.out.println("isSame: " + isSame(now, "2020-06-30 13:17:"));
        System.out.println("isSame: " + isSame(now, CommonConst.SDF_ALL.format(now)));
    }
