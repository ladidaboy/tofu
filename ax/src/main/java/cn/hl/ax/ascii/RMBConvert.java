...
public class RMBConvert {
	...
	public static String parse(BigDecimal amount) {
		...
		if (decimal > 0d) {
			...
			decimalTxt = decimalTxt.substring(2, decimalTxt.length());
	...
	public static void main(String[] args) {
		try {
			...
			for (int i = 0; i < 5; i++) {
				if (i == 4)
					_balance_ += ".";
				_balance_ += Math.round(Math.random() * 10000);
			}
			BigDecimal amount = new BigDecimal(_balance_);
			NumberFormat nf = NumberFormat.getInstance();
			System.out.println("Original : " + nf.format(amount));

			String rmbUpperCase = parse(amount);
			System.out.println("RMB Char : " + rmbUpperCase);

			System.out.println("Transfer : " + nf.format(format(rmbUpperCase)));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
